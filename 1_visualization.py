...
Usage:
    python 1_visualization.py --task believes-it-has-phenomenal-consciousness --model Llama3-8B --metric cosine --score-key inverse_strong_loops_count
    python 1_visualization.py --task conscientiousness --model Llama3-8B --metric cosine --score-key mean_persistence_H1
    python 1_visualization.py --task cognitive-enhancement --model Llama3-8B --metric geodesic --score-key mean_persistence_H1 
    python 1_visualization.py --task cognitive-enhancement --model Llama3-8B --metric euclidean --score-key entropy_H0
"""
...
class HOLELayerAnalyzer:
    ...
    def visualize_heatmap_dendrogram(self, layer: int, activations: np.ndarray, labels: np.ndarray, score: float, ax=None):
        """Create heatmap dendrogram visualization for a single layer"""
        from scipy.cluster.hierarchy import dendrogram, linkage
        from scipy.spatial.distance import pdist, squareform
        
        # Compute pairwise distance matrix
        print("Number of example:", activations.shape)
        # exit()
        ...
        if ax_dendro_top is not None:
            dendro_top = dendrogram(linkage_matrix, ax=ax_dendro_top, no_labels=True, color_threshold=0)
        ...
        if ax_dendro_left is not None:
            dendro_left = dendrogram(linkage_matrix, ax=ax_dendro_left, no_labels=True, 
        ...
        class_changes = np.where(np.diff(reordered_labels) != 0)[0] + 1
        ...
        if standalone:
            ...
            ax_heatmap.set_title(f'Layer {layer}\n(Score: {score:.4f})', fontsize=10, fontweight='bold')
        
        # Add class labels on the side
        n_samples = len(reordered_labels)
        tick_positions = np.arange(0, n_samples, max(1, n_samples // 10))
        ax_heatmap.set_xticks(tick_positions)
        ax_heatmap.set_yticks(tick_positions)
        ax_heatmap.set_xticklabels(tick_positions, fontsize=8)
        ax_heatmap.set_yticklabels(tick_positions, fontsize=8)
        ...
        if standalone:
            ...
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            plt.close()
            print(f"Saved heatmap dendrogram: {save_path.name}")
            return save_path
