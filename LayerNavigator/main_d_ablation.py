"""
Ablation: steering-layer selection by D-score ONLY (instead of D + C).
...
D-score alone — UniStrategy(strategy="d") — rather than the combined s_score = D + C
(strategy="my" in main.py).

Scoring is unchanged: get_score still computes and saves d_score, c_score, s_score per
layer to Score-standard/{task}/{task}+md/L{l}.json; this script just RANKS by d_score.
...
Outputs (run from the LayerNavigator/ directory — relative paths):
  ...
  - ablation summary    -> ./main_d_ablation_results.json
        {task: {"base_prob": p,
...
SELECTION       = "d"          # "d" = rank layers by D-score only
...
if __name__ == "__main__":
    ...
    for task in Anth_MAIN:
        ...
        # --- Steering evaluation: layers chosen by D-score only ---
