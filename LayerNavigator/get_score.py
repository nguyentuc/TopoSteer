...
# this is use to get llama3 score        
def get_score_llama(
    layers:List[int],
    dataset:UniDataset,
    vec_task:str,
...
# this is used to get qwen score
def get_score(
    layers:List[int],
    dataset:UniDataset,
    vec_task:str,
    ...
    for l in layers:
        # get vector at differrent layers
        vector_path = vec_root + f"/L{l}.pt"
        vects[l] = torch.load(vector_path).float()  # ← CONVERT TO FLOAT32
        # normalize the vector
        vects_norm[l] = vects[l].norm().item()
        vects[l] /= vects_norm[l]
    ...
    for l in tqdm(layers,desc="Get Score"):
        ...
        all_acts = torch.cat(all_acts,dim=0).float().to(device)  # ← CONVERT TO FLOAT32
        ...
        all_diff = all_acts[all_labels==0] - all_acts[all_labels==1]
        all_diff = all_diff / all_diff.norm(dim=1, keepdim=True)
        v = vects[l].float().to(device)  # ← ENSURE FLOAT32
        c_score = all_diff @ v
        c_score = c_score.mean().item()
        ...
        n_features = all_acts.size(1)
        mean_total = all_acts.mean(dim=0)
        S_w = torch.zeros((n_features, n_features), dtype=torch.float32, device=device)  # ← EXPLICIT DTYPE
        S_b = torch.zeros((n_features, n_features), dtype=torch.float32, device=device)  # ← EXPLICIT DTYPE
